...
Inicie a API com `npm --prefix API run start:prod` e visualize o portal compilado
com `npm --prefix FrontEnd run preview` em **http://localhost:4173/**.
Para hospedagem, sirva `FrontEnd/dist` e encaminhe `/auth`, `/users`, `/medicines`
e `/uploads` para a API através do proxy do servidor. O proxy do Vite é para
uso local. As fotos enviadas ficam em `API/uploads` ou em `UPLOAD_DIR`; use um
volume persistente ao hospedar a API.
